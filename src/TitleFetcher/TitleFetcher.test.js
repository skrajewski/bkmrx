...
test('it should throw error if fetcher is unable to retrieve title', () => {
    ...
    return expect(fetchTitle('http://plecom')).rejects.toStrictEqual(new Error('Dummy error'))
});

test('it should throw error if fetcher html contains to title tag', async () => {
    fetch.mockReturnValue(Promise.resolve(new Response('<head><head><body><h1>Hello World</h1></body>')));

    expect.assertions(3);

    try {
        await fetchTitle('http://example.com');
    } catch (e) {
        expect(e).toStrictEqual(new Error('Unable to parse the title tag'));
    }
